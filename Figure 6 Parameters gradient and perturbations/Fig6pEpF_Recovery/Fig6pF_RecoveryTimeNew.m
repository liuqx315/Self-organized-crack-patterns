...
FS=22;
% bardat=[46 89];
bardat=[32.4991136789322 24.0205749869347; 46 89];
X = categorical({'Rescaled biomass','Recovery time'});
X = reordercats(X,{'Rescaled biomass','Recovery time'});
...
% xlabel('Time, $t$','fontsize',FS,'Interpreter','latex')
ylabel(tp,'Recovery time and biomass at $\mathcal{P}_{e}$','fontsize',FS,'Interpreter','latex')
ytickformat('%1.0f')
% xlim([0 250])
ylim([0.0 100])
legend('Patterns intact', 'Homogeneity', 'location','northwest','box','off')
set(gca,'linewidth',2,'fontsize',FS,'ticklength',[0.020 0.015],'xminortick','off')
%
% % subplot(2,1,2)
% nexttile
